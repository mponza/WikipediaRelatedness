...
import it.unimi.dsi.law.rank.{PageRankParallelGaussSeidel, PageRankPowerSeries}
...
class PowerPageRanker (val wikiID: Int, val wikiBVGraph: WikiBVGraph, val alpha: Float) extends WebGraphPageRanker {
  protected val logger = LoggerFactory.getLogger(getClass)

  protected var prior = Array.ofDim[Double]( wikiBVGraph.graph.numNodes() )
  prior( wikiBVGraph.getNodeID(wikiID) ) = 1.0

  protected val ppr = new PageRankPowerSeries( wikiBVGraph.graph )
  ppr.alpha = alpha
