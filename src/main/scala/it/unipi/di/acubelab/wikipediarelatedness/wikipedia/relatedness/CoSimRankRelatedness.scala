...
import it.unipi.di.acubelab.wikipediarelatedness.wikipedia.processing.webgraph.algorithms.CoSimRank
...
class CoSimRankRelatedness(options: CoSimRankOptions) extends Relatedness {
  ...
  val csr = new CoSimRank(WikiGraphFactory.outGraph, options.iterations, options.pprDecay, options.csrDecay)
