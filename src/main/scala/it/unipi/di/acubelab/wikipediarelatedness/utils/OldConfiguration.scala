...
object OldConfiguration {
  ...
  val topKEmbeddings = Map(
    "sg" -> Paths.get(projDir, "/data/processing/wikipedia/topKembeddings/sg").toString,
    "dwsg" -> Paths.get(projDir, "/data/processing/wikipedia/topKembeddings/dwsg").toString
  )
